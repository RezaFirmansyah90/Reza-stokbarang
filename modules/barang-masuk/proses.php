...
// jika user sudah login, maka jalankan perintah untuk insert, update, dan delete
    ...
    if ($_GET['act']=='insert') {
        if (isset($_POST['simpan'])) {
            ...
            $query = mysqli_query($mysqli, "INSERT INTO is_obat_masuk(kode_transaksi,tanggal_masuk,kode_helm,jumlah_masuk,created_user) 
                                            VALUES('$kode_transaksi','$tanggal_masuk','$kode_helm','$jumlah_masuk','$created_user')")
                                            or die('Ada kesalahan pada query insert : '.mysqli_error($mysqli));    

            // cek query
            if ($query) {
                // perintah query untuk mengubah data pada tabel helm
                $query1 = mysqli_query($mysqli, "UPDATE is_helm SET stok        = '$total_stok'
